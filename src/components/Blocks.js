...
export const ChecklistBlock = ({ block, updateBlock, isEditMode }) => {
    ...
    // --- GROUPING LOGIC FOR INTERACTIVE MODE ---
    ...
    // Sort categories alphabetically, pushing blanks ("Uncategorized") to the bottom
                                ...
                                <tr key={row.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                    <td style={{ padding: '8px 10px' }}>
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                                            <input type="text" placeholder="Task..." value={row.task} onChange={(e) => handleUpdateRow(row.id, 'task', e.target.value)} style={{ width: '100%', border: '1px solid #cbd5e1', padding: '6px', borderRadius: '4px' }} />
                                            <input type="text" placeholder="Category (e.g. Yard)" value={row.category || ''} onChange={(e) => handleUpdateRow(row.id, 'category', e.target.value)} style={{ width: '100%', border: '1px solid #cbd5e1', padding: '4px 6px', borderRadius: '4px', fontSize: '12px', backgroundColor: '#f8fafc' }} />
                                        </div>
                                    </td>
                                    <td style={{ padding: '8px 10px' }}>
                                        <input type="number" value={row.value} onChange={(e) => handleUpdateRow(row.id, 'value', e.target.value)} step="0.25" style={{ width: '100%', border: '1px solid #cbd5e1', padding: '6px', textAlign: 'right', borderRadius: '4px' }} />
                                    </td>
                                    {row.days.map((_, index) => (
                            ...
                            sortedCats.map(cat => {
                                ...
                                const itemRows = groupedRows[cat].map(row => (
                                    <tr key={row.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                        ...
                                        {row.days.map((isChecked, index) => (
                                            <td key={index} style={{ padding: '8px 10px', textAlign: 'center' }}>
                                                <input type="checkbox" checked={isChecked} onChange={() => handleToggleDay(row.id, index)} style={{ transform: 'scale(1.2)', cursor: 'pointer' }} />
                                            </td>
                                        ))}
